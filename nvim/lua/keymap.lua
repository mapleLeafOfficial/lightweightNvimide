...
G.g.mapleader = " " -- Make sure to set `mapleader` before lazy so your mappings are correct
...
-- 检查是否为 Linux 系统
if G.fn.has('unix') == 1 then
  -- 设置 XKB 选项，将 Caps Lock 映射为 Escape
  G.fn.system('setxkbmap -option "caps:escape"')
end
-- 光标在$ 0 ^依次跳转
function MagicMove()
    local first = 1
